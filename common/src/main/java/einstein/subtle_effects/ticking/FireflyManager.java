...
public class FireflyManager {
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, RandomSource random) {
        ...
        if (dimensionKey.isEmpty() || ENVIRONMENT.fireflies.dimensionBlocklist.get().contains(dimensionKey.get().identifier())) {
            return;
        }
        ...
        if (!ENVIRONMENT.fireflies.biomesBlocklist.get().contains(biomeId)) {
            ...
            if (biome.value().warmEnoughToRain(pos, level.getSeaLevel()) || ENVIRONMENT.fireflies.biomesAllowlist.get().contains(biomeId)) {
